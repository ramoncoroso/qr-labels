defmodule QrLabelSystemWeb.CoreComponents do
  @moduledoc """
  ...
  def flash(assigns) do
    ...
    <div
      ...
      role="alert"
      class={[
        "fixed top-16 left-1/2 -translate-x-1/2 w-auto max-w-lg z-50 rounded-xl px-6 py-4 ring-1 shadow-xl transition-opacity duration-300",
        @kind == :info && "bg-emerald-50 text-emerald-800 ring-emerald-500 fill-cyan-900",
        @kind == :error && "bg-rose-50 text-rose-900 shadow-md ring-rose-500 fill-rose-900"
      ]}
      {@rest}
    >
      <p :if={@title} class="flex items-center gap-2 text-base font-bold leading-6">
      ...
      <p class="mt-2 text-base font-semibold leading-5"><%= msg %></p>
  ...
  Icons are extracted from the `deps/heroicons` package.
  ...
  def translate_error({msg, opts}) do
    if count = opts[:count] do
      Gettext.dngettext(QrLabelSystemWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(QrLabelSystemWeb.Gettext, "errors", msg, opts)
    end
  end
