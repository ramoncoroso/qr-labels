defmodule QrLabelSystemWeb.GenerateLive.DataFirst do
  ...
  # Received from DataFileReader hook after client-side parsing + IndexedDB storage
  @impl true
  def handle_event("client_file_read", %{"columns" => cols, "total_rows" => total, "sample_rows" => rows}, socket) do
    user_id = socket.assigns.current_user.id
    design_id = socket.assigns.design_id

    # Store metadata in ETS
    QrLabelSystem.UploadDataStore.put_metadata(user_id, design_id, cols, total, rows)

    {:noreply,
     socket
     |> assign(:upload_columns, cols)
     |> assign(:upload_total_rows, total)
     |> assign(:upload_sample_rows, rows)
     |> assign(:upload_error, nil)
  ...
  def handle_event("parse_pasted", _params, socket) do
    ...
    if String.trim(text) == "" do
      {:noreply, assign(socket, :upload_error, "No hay datos para procesar")}
    else
      # Send text to client-side hook for parsing + IndexedDB storage
      {:noreply, push_event(socket, "process_paste_client", %{text: text})}
    end
  end

  @impl true
  def handle_event("continue", _params, socket) do
    if socket.assigns.upload_total_rows > 0 do
      design_id = socket.assigns.design_id

      # Metadata already in UploadDataStore (stored in client_file_read handler)
      # Data already in IndexedDB (stored by DataFileReader hook)
  ...
  def handle_event("continue_no_data", _params, socket) do
    # Clear any existing metadata for this user
    user_id = socket.assigns.current_user.id
  ...
  def render(assigns) do
    ...
    <div id="data-first-page" class="max-w-4xl mx-auto" phx-hook="ScrollTo">
      ...
      <div class="mt-8">
        ...
        <!-- Upload Area (Excel/CSV) — client-side via DataFileReader hook -->
        <div
          :if={@active_method == "file"}
          id="data-file-reader"
          phx-hook="DataFileReader"
        ...
        >
          ...
          >
            ...
            <input type="file" accept=".xlsx,.xls,.csv,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,application/vnd.ms-excel,text/csv,text/plain" class="sr-only" />
          </label>
        </div>
        ...
        <!-- Paste Area -->
        <div
          :if={@active_method == "paste"}
          id="data-paste-reader"
          phx-hook="DataFileReader"
          data-user-id={@current_user.id}
          data-design-id={@design_id}
          class="bg-white rounded-xl shadow-sm border border-gray-200 p-6"
        ...
        <%= if @upload_total_rows > 0 do %>
          <div id="data-preview" class="mt-8 bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
              ...
              <div class="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p class="text-sm text-green-700">
                    <%= @upload_total_rows %> registros, <%= length(@upload_columns) %> columnas
                  </p>
                </div>
              </div>
            </div>
            ...
            <div class="p-6">
              ...
              <%= if @upload_total_rows > 5 do %>
                <p class="mt-3 text-sm text-gray-500 text-center">
                  ...y <%= @upload_total_rows - 5 %> filas más
                </p>
              <% end %>
            </div>
          </div>
        ...
        <div :if={@upload_total_rows == 0} class="mt-8">
          <button
            phx-click="back"
